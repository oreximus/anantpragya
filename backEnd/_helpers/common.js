...
module.exports = {
  ...
  getVatAmount: (amount = 0) =>
    amount > 0 ? amount - amount * (100 / (100 + config.app.vat)) : 0,
  ...
  getDiscountAmount: function (price = 0, discount = 0) {
    return discount && discount > 0
      ? this.getExVatAmount(price) * (discount / 100)
      : 0;
  },
  // Calculate discount without VAT amount
  calculateDiscount: (amount = 0, discount = 0) =>
    discount && discount > 0 ? amount * (discount / 100) : 0,
  // Calculate VAT without VAT price
  calculateVat: (amount = 0, percentage = 0) =>
    percentage > 0 ? amount * (percentage / 100) : 0,
  // Calculate Percentage (%)
  calculatePercentage: (amount = 0, percentage = 0) =>
    percentage > 0 ? amount * (percentage / 100) : 0,
