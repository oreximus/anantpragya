...
function userRegister(req, res, next) {
  ...
  const schema = Joi.object().keys({
    ...
    password: Joi.string().pattern(PASSWORD_REGEX).trim(true).min(6).max(20).required().messages({
...
function changePassword(req, res, next) {
  ...
  const schema = Joi.object().keys({
    ...
    new_password: Joi.string().pattern(PASSWORD_REGEX).trim(true).min(6).max(20).required().messages({
      "string.pattern.base": `Password must be at least 6 characters long and contain at least one uppercase letter, one lowercase letter, one special character, and one number!`,
    }),
    confirm_password: Joi.string()
